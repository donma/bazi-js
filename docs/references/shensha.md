...
- 規則名稱：一般神煞判定引擎（Data-driven）
- ruleId：SS_*（每條見 shensha-catalog.md）
- 採用方法：每條一般神煞為一筆資料（含 id/name/category/baseOn/match/ruleId/version/reference），
  引擎掃描四柱逐條比對，命中輸出 hitOn/basedOn/evidence。固定日柱與季節特殊條件已移至 SpecialRules。
...
- 分類邊界：固定日柱/時柱與季節特殊條件見 `classical-special-rules.md`；整局特殊格見 `src/patterns/`，不混入 ShenSha Catalog。
