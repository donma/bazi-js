...
11. **節氣時刻**：各軟體內建表互差數分鐘，屬曆算精度非流派；Round 03 已確認目前低精度模型在部分分鐘邊界可能提前約 7～8 分鐘，見 solar-terms.md 與差異裁決報告。
12. **大運方向**：另有不分陰陽一律順排之簡派；canonical 採陰陽年＋性別。
13. **夏令時間**：BaziJS 目前要求使用者自行提供正確時區，尚未內建歷史政治時區／DST 資料庫；外部帶 `dstOffset` 的結果不得直接視為 BaziJS 已支援。
14. **午夜晚子時時干**：`00:00` Profile 可有同日五鼠遁與「日用本日、時用明日之干」兩種處理，必須以 Profile 明確指定，不覆寫 canonical。
